...
pub fn draw_move_gizmo(
    ...
    let get_color = |handle: GizmoHandle, default_color: Color32| -> Color32 {
    ...
    let x_color = get_color(GizmoHandle::MoveX, config.color_x_axis);
    ...
    let y_color = get_color(GizmoHandle::MoveY, config.color_y_axis);
    ...
    let center_color = get_color(GizmoHandle::MoveXY, config.color_center);
    let half_size = config.center_handle_size / 2.0;
