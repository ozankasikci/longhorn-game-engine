...
use engine_renderer_3d::{
    Camera, EcsRendererIntegration, FrustumCuller, RenderQueue, Renderer3D, SortMode,
};
use glam::Vec3;
use std::sync::Arc;
...
) -> Result<(Renderer3D, Camera, World, EcsRendererIntegration), Box<dyn std::error::Error>> {
...
async fn test_complete_pipeline(
    ...
    println!(
        "      1. ECS → Scene: {:.2}ms ({} objects)",
        ecs_time.as_secs_f64() * 1000.0,
        scene.objects.len()
    );
    ...
    println!(
        "      4. Material Grouping: {:.2}ms ({} groups)",
    ...
    if !material_groups.is_empty() {
        ...
        for (i, group) in material_groups.iter().enumerate() {
            println!(
                "      Group {}: material_id={}, objects={}, range={:?}",
                i + 1,
                group.material_id,
                group.count,
                group.range()
...
async fn performance_benchmark(
    ...
    for run in 0..num_runs {
        ...
        if run == 0 {
            ...
            let queue_stats = render_queue.get_stats();
            println!(
                "      Run 1 details: {:.2}ms, culling: {}, queue: {}",
...
async fn test_realtime_updates(
    ...
    for i in 0..100 {
        let entity = world.spawn();
        let initial_transform = Transform::new().with_position(
        ...
        world
            .add_component(entity, initial_transform.clone())
            .unwrap();
        world
            .add_component(
                entity,
