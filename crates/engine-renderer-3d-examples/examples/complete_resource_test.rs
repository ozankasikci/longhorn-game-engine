...
use engine_renderer_3d::{
    Renderer3D, RenderScene, RenderObject, Camera, Mesh, Material, 
    TextureDescriptor, create_test_pattern
};
use glam::{Mat4, Vec3};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   📈 Resource Growth: +{} meshes, +{} materials, +{} textures",
    ...
    let scene_objects = vec![
        ...
        // Right: Blue triangle
        ("Blue Triangle", RenderObject::new(
            Mat4::from_translation(Vec3::new(3.0, 0.0, 0.0)) * 
    ...
    for (resource_type, actual, expected) in validation_results {
        ...
        println!("   {} {}: {} (expected {})", status, resource_type, actual, expected);
        if actual != expected {
            all_valid = false;
        }
    }
    ...
    println!("======================");
    println!("Resource Management System: {}", if all_valid { "✅ PASSED" } else { "❌ FAILED" });
...
fn create_sphere_mesh(sectors: u32, stacks: u32) -> Mesh {
    ...
    Mesh::new("Sphere".to_string(), vertices, indices.into_iter().map(|i| i as u16).collect())
}
...
fn create_plane_mesh(size: f32) -> Mesh {
    ...
    let vertices = vec![
        Vertex { position: [-half_size, 0.0, -half_size], color: [0.9, 0.9, 0.9] },
        Vertex { position: [ half_size, 0.0, -half_size], color: [0.9, 0.9, 0.9] },
        Vertex { position: [ half_size, 0.0,  half_size], color: [0.9, 0.9, 0.9] },
        Vertex { position: [-half_size, 0.0,  half_size], color: [0.9, 0.9, 0.9] },
    ];
