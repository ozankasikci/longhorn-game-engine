...
fn test_basic_camera_controller() {
    ...
    println!(
        "   ✅ View matrix: determinant = {:.2}",
        view_matrix.determinant()
    );
    println!(
        "   ✅ Projection matrix: determinant = {:.2}",
        proj_matrix.determinant()
    );
    println!(
        "   ✅ View-Projection matrix: determinant = {:.2}",
        vp_matrix.determinant()
    );
...
fn test_matrix_caching() {
    ...
    println!(
        "      • Cached access (1000 calls): {:.2}ms",
        cached_time.as_secs_f64() * 1000.0
    );
    println!(
        "      • With recalculation (100 calls): {:.2}ms",
        recalc_time.as_secs_f64() * 1000.0
    );
    println!(
        "      • Average cached call: {:.4}ms",
        cached_time.as_secs_f64() * 1.0
    );
    println!(
        "      • Average recalc call: {:.4}ms",
        recalc_time.as_secs_f64() * 10.0
    );
...
fn test_camera_controls() {
    ...
    println!(
        "      ✅ Forward movement: moved {:.2} units",
        (forward_pos - initial_pos).length()
    );
    ...
    println!(
        "      ✅ Right movement: moved {:.2} units",
        (right_pos - forward_pos).length()
    );
    ...
    println!(
        "      ✅ Up movement: moved {:.2} units",
        (up_pos - right_pos).length()
    );
    ...
    println!(
        "      ✅ Orbital movement: maintained distance {:.2}",
        final_distance
    );

    // Test zoom
    let zoom_initial_distance = controller
        .camera
        .position
        .distance(controller.camera.target);
    controller.zoom(-2.0); // Zoom in
    let zoom_final_distance = controller
        .camera
        .position
        .distance(controller.camera.target);
    ...
    println!(
        "      ✅ Zoom: distance changed from {:.2} to {:.2}",
...
fn test_ray_casting() {
    ...
    if let Some(distance) = test_ray.intersect_sphere(sphere_center, sphere_radius) {
        ...
        println!(
            "      🎯 Ray-sphere intersection at distance {:.2}, point {:?}",
