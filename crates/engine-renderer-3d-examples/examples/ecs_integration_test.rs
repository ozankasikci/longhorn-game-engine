...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, obj) in render_scene.objects.iter().enumerate() {
        ...
        println!(
            "   Object {}: mesh_id={}, material_id={}, pos={:?}",
            i + 1,
            obj.mesh_id,
            obj.material_id,
            translation
    ...
    println!(
        "   ➕ Added new entity: {} at position (5, 1, 0)",
        new_entity.id()
    );
    ...
    println!(
        "      - Objects: {} (was {})",
        updated_scene.objects.len(),
        render_scene.objects.len()
    );
    println!(
        "      - New object count: +{}",
        updated_scene.objects.len() - render_scene.objects.len()
    );

    // Phase 6: Custom Camera Test
    println!("\n📸 Phase 6: Custom Camera Test");
    let custom_camera =
        Camera::from_position_rotation([3.0, 4.0, 5.0], [0.1, 0.2, 0.0], aspect_ratio);
    let custom_scene = integration.world_to_scene_with_camera(&world, custom_camera);

    println!("   📊 Custom camera scene:");
    println!(
        "      - Camera position: {:?}",
        custom_scene.camera.position
    );
    ...
    println!(
        "      - Renderable entities in ECS: {}",
        expected_renderable_entities
    );
    ...
    println!(
        "      - Material mappings: {}",
        mapping_stats.material_mappings
    );
    println!("      - Default mesh ID: {}", mapping_stats.default_mesh_id);
    println!(
        "      - Default material ID: {}",
        mapping_stats.default_material_id
    );
...
fn create_test_entities(world: &mut World) -> Vec<(engine_ecs_core::ecs_v2::Entity, String)> {
    ...
    world
        ...
        .unwrap();
    world
        .add_component(entity1, EcsMaterial::default())
        .unwrap();
    ...
    world
        .add_component(entity2, Transform::new().with_position(2.0, 0.0, 0.0))
        .unwrap();
    world
        .add_component(
            entity2,
        ...
        .unwrap();
    world
        .add_component(entity2, EcsMaterial::default())
        .unwrap();
    ...
    world
        .add_component(entity3, Transform::new().with_position(-2.0, 1.0, 0.0))
        .unwrap();
    world
        .add_component(
            entity3,
        ...
        .unwrap();
    world
        .add_component(entity3, EcsMaterial::default())
        .unwrap();
    ...
    world
        .add_component(entity4, Transform::new().with_position(0.0, 2.0, 0.0))
        .unwrap();
    world
        .add_component(
            entity4,
            Mesh {
                mesh_type: MeshType::Custom("pyramid".to_string()),
            },
        )
        .unwrap();
    world
        .add_component(entity4, EcsMaterial::default())
        .unwrap();
    ...
    world
        .add_component(entity5, Transform::new().with_position(10.0, 10.0, 10.0))
        .unwrap();
...
fn count_renderable_entities(world: &World) -> usize {
    ...
    let transform_entities: HashSet<_> =
        world.query_legacy::<Transform>().map(|(e, _)| e).collect();
    let mesh_entities: HashSet<_> = world.query_legacy::<Mesh>().map(|(e, _)| e).collect();
    let material_entities: HashSet<_> = world
        .query_legacy::<EcsMaterial>()
        .map(|(e, _)| e)
        .collect();
