...
use engine_renderer_3d::{BoundingVolume, Camera, FrustumCuller, RenderObject};
use glam::{Mat4, Vec3};
...
fn test_object_position_culling() {
    ...
    let test_objects = vec![
        ...
        (
            create_test_object(Vec3::new(0.0, 0.0, -50.0)),
            "Far away (might be visible)",
        ),
        (
            create_test_object(Vec3::new(0.0, 0.0, 5.0)),
        ...
        (
            create_test_object(Vec3::new(100.0, 0.0, -5.0)),
            "Far right (should be culled)",
        ),
        (
            create_test_object(Vec3::new(-100.0, 0.0, -5.0)),
            "Far left (should be culled)",
        ),
        (
            create_test_object(Vec3::new(0.0, 100.0, -5.0)),
            "Far up (should be culled)",
        ),
        (
            create_test_object(Vec3::new(0.0, -100.0, -5.0)),
            "Far down (should be culled)",
        ),
        (
            create_test_object(Vec3::new(2.0, 0.0, -5.0)),
            "Slightly right (might be visible)",
        ),
        (
            create_test_object(Vec3::new(-2.0, 0.0, -5.0)),
            "Slightly left (might be visible)",
        ),
    ];
...
fn test_bounding_volume_culling() {
    ...
    let test_cases = vec![
        ...
        (
            create_test_object(Vec3::new(0.0, 0.0, -5.0)),
            BoundingVolume::aabb(Vec3::new(-1.0, -1.0, -6.0), Vec3::new(1.0, 1.0, -4.0)),
    ...
    let objects_with_bounds: Vec<_> = test_cases
        .iter()
