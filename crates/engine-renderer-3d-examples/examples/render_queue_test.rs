...
use engine_renderer_3d::{RenderQueue, RenderObject, Camera, SortMode};
...
fn test_sorting(sort_mode: SortMode, camera: &Camera) {
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        println!("      Object {}: distance={:.1}, pos={:?}", 
                 i + 1, item.camera_distance, pos);
    }
    ...
    match sort_mode {
        SortMode::FrontToBack => {
            for i in 1..items.len() {
                assert!(items[i-1].camera_distance <= items[i].camera_distance,
...
fn test_material_sorting(camera: &Camera) {
    ...
    for (i, item) in items.iter().enumerate() {
        println!("      Object {}: material_id={}", i + 1, item.object.material_id);
    }
    
    // Verify material sorting
    for i in 1..items.len() {
        assert!(items[i-1].object.material_id <= items[i].object.material_id,
    ...
    for (i, group) in groups.iter().enumerate() {
        println!("      Group {}: material_id={}, count={}, range={:?}",
...
fn test_material_distance_sorting(camera: &Camera) {
    ...
    for (i, item) in items.iter().enumerate() {
        let pos = item.object.transform.col(3).xyz();
        println!("      Object {}: material_id={}, distance={:.1}, pos={:?}",
                 i + 1, item.object.material_id, item.camera_distance, pos);
    }
    ...
    for i in 1..items.len() {
        ...
        if prev.object.material_id == curr.object.material_id {
            ...
            assert!(prev.camera_distance <= curr.camera_distance,
                    "Distance sorting within material failed");
        } else {
            // Different material, material ID should be sorted
            assert!(prev.object.material_id <= curr.object.material_id,
                    "Material sorting failed");
        }
    }
...
fn test_performance(camera: &Camera) {
    ...
    for sort_mode in [SortMode::None, SortMode::Material, SortMode::FrontToBack, SortMode::MaterialThenDistance] {
        let mut queue = RenderQueue::new(sort_mode);
        ...
        println!("      {:?}: add={:.2}ms, sort={:.2}ms, draw_calls={}",
                 sort_mode, 
