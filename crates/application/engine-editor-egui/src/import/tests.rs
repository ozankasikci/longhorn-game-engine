...
fn test_drag_drop_import() {
    ...
    assert_eq!(drag_drop.detect_file_type(&dropped_files[0]), Some(FileType::Mesh));
    assert_eq!(drag_drop.detect_file_type(&dropped_files[1]), Some(FileType::Texture));
    assert_eq!(drag_drop.detect_file_type(&dropped_files[2]), Some(FileType::Audio));
}
...
fn test_batch_import() {
    ...
    use {BatchImporter, BatchImportOptions};
...
fn test_import_hot_reload() {
    ...
    use {HotReloadWatcher, HotReloadEvent};
