...
use engine_mesh_import::{ObjImporter, MeshData};
use engine_texture_import::{TextureImporter, TextureData};
use engine_audio_import::{AudioImporter, AudioData};
...
impl<I: AssetImporter<Asset = MeshData> + Send + Sync> AssetImporter for MeshImporterWrapper<I> {
    ...
    async fn import(&self, path: &Path, context: &ImportContext) -> ImportResult<Self::Asset> {
        ...
        bincode::serialize(&mesh_data)
            .map_err(|e| ImportError::ProcessingError(format!("Failed to serialize mesh data: {}", e)))
    }
...
#[async_trait]
impl<I: AssetImporter<Asset = TextureData> + Send + Sync> AssetImporter for TextureImporterWrapper<I> {
    type Asset = Vec<u8>;
    ...
    async fn import(&self, path: &Path, context: &ImportContext) -> ImportResult<Self::Asset> {
        ...
        // Serialize to bytes using bincode
        bincode::serialize(&texture_data)
            .map_err(|e| ImportError::ProcessingError(format!("Failed to serialize texture data: {}", e)))
    }
...
impl<I: AssetImporter<Asset = AudioData> + Send + Sync> AssetImporter for AudioImporterWrapper<I> {
    ...
    async fn import(&self, path: &Path, context: &ImportContext) -> ImportResult<Self::Asset> {
        ...
        // Serialize to bytes using bincode
        bincode::serialize(&audio_data)
            .map_err(|e| ImportError::ProcessingError(format!("Failed to serialize audio data: {}", e)))
    }
...
impl SerializedAssetData {
    ...
    pub fn deserialize_mesh(&self) -> Result<MeshData, bincode::Error> {
        match self {
            ...
            _ => Err(Box::new(bincode::ErrorKind::Custom("Not mesh data".to_string()))),
        }
    }
    ...
    pub fn deserialize_texture(&self) -> Result<TextureData, bincode::Error> {
        match self {
            ...
            _ => Err(Box::new(bincode::ErrorKind::Custom("Not texture data".to_string()))),
        }
    }
    ...
    pub fn deserialize_audio(&self) -> Result<AudioData, bincode::Error> {
        match self {
            ...
            _ => Err(Box::new(bincode::ErrorKind::Custom("Not audio data".to_string()))),
        }
    }
}
...
mod tests {
    ...
    fn test_serialized_asset_data() {
        use engine_mesh_import::types::{Vertex, Material};
