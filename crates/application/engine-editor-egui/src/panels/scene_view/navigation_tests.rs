...
mod camera_rotation_tests {
    ...
    fn test_mouse_look_horizontal_rotation() {
        ...
        assert!(final_yaw < initial_yaw, "Camera should rotate left when mouse moves right");
        ...
        assert!(scene_nav.rotation_velocity[1] < 0.0, "Yaw velocity should be negative for right movement");
    }
    ...
    fn test_mouse_look_vertical_rotation() {
        ...
        assert!(final_pitch < initial_pitch, "Camera should pitch down when mouse moves down");
        assert_eq!(scene_nav.scene_camera_transform.rotation[1], 0.0); // Yaw unchanged
        
        // Verify the rotation velocity was set up correctly
        assert!(scene_nav.rotation_velocity[0] < 0.0, "Pitch velocity should be negative for down movement");
    }
    ...
    fn test_rotation_sensitivity_affects_rotation_speed() {
        ...
        assert!(high_velocity > low_velocity, "High sensitivity should create higher velocity");
        ...
        assert!(velocity_ratio > 2.0 && velocity_ratio < 20.0, 
                "Velocity ratio should be roughly proportional to sensitivity. Got: {}", velocity_ratio);
    }
}
...
mod camera_rotation_feel_tests {
    ...
    fn test_smooth_rotation_acceleration() {
        ...
        assert!(has_variation, 
                "Rotation should have smooth acceleration/deceleration. Got: {:?}", rotations);
    }
    ...
    fn test_adaptive_sensitivity_based_on_movement_speed() {
        ...
        assert!(velocity_ratio < 10.0, 
                "Fast movement should have dampened sensitivity. Got ratio: {}", velocity_ratio);
        assert!(velocity_ratio > 5.0, 
                "But should still be significantly faster than slow movement. Got ratio: {}", velocity_ratio);
    }
    ...
    fn test_mouse_smoothing_reduces_jitter() {
        ...
        assert!(variance < 0.01, "Mouse smoothing should reduce rotation variance");
    }
    ...
    fn test_rotation_interpolation_for_smooth_camera() {
        ...
        assert!(rotation_diff <= max_single_frame_rotation,
...
mod camera_navigation_integration_tests {
    ...
    fn test_rotation_consistency_across_frames() {
        ...
        assert!(total_yaw < 0.0, "Total yaw should be negative for rightward mouse movement");
        assert!(total_pitch < 0.0, "Total pitch should be negative for downward mouse movement");
        
        // Verify the rotation is significant enough to be useful (lower threshold for velocity-based system)
        assert!(total_yaw.abs() > 0.0001, "Should have significant yaw rotation");
        assert!(total_pitch.abs() > 0.0001, "Should have significant pitch rotation");
    }
}