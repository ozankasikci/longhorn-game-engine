...
fn test_complete_object_creation_workflow() {
    ...
    use engine_editor_framework::{EditorState, world_setup};
    ...
    world.add_component(entity, engine_components_ui::Name::new("TestCube")).unwrap();
...
fn test_complete_asset_loading_workflow() {
    ...
    use engine_editor_assets::{TextureManager, create_default_textures};
...
fn test_play_mode_transition_workflow() {
    ...
    use engine_editor_ui::{Toolbar, PlayState as UIPlayState};
...
fn test_console_message_workflow() {
    ...
    use engine_editor_panels::{ConsolePanel, ConsoleMessage as PanelMsg};
...
fn test_error_handling_workflow() {
    ...
    use engine_editor_panels::{ConsolePanel, ConsoleMessage};
    ...
    console.add_messages(vec![
        ConsoleMessage::error(&format!("Asset load failed: {}", error))
    ]);
    ...
    match &console.console_messages[0] {
        ConsoleMessage::Message { message_type, .. } => {
            assert!(matches!(message_type, engine_editor_panels::ConsoleMessageType::Error));
        }
