    ...
    let file_count = std::fs::read_dir(scene_view_path)
        ...
        .filter(|entry| {
            entry.as_ref().unwrap().path().extension()
                .and_then(|s| s.to_str()) == Some("rs")
        })
        .count();
    
    println!("Found {} files in scene_view module", file_count);
    assert!(file_count >= 10, "Should have at least 10 files in scene view");
}
...
fn test_scene_view_line_count() {
    ...
    for entry in std::fs::read_dir(scene_view_path).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("rs") {
            ...
            total_lines += lines;
            println!("{}: {} lines", path.file_name().unwrap().to_string_lossy(), lines);
        }
    }
...
fn test_scene_view_dependencies() {
    ...
    let has_parent_imports = scene_view_mod.contains("use super::") || 
                            scene_view_mod.contains("use crate::");
...
fn test_scene_view_removed() {
    ...
    let scene_view_path = Path::new("src/panels/scene_view");
    assert!(!scene_view_path.exists(), "Scene view should be moved to separate crate");
}
...
fn test_scene_view_interface_exists() {
    ...
    assert!(content.contains("trait SceneViewPanel") || 
