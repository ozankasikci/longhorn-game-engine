...
fn test_editor_panels_extraction_structure() {
    ...
    assert!(panels_path.exists(), "engine-editor-panels crate should exist");
    ...
    for file in &expected_panels {
        ...
        assert!(file_path.exists(), "Panel file {} should exist in new crate", file);
    }
}
...
fn test_editor_ui_extraction_structure() {
    ...
    for file in &expected_ui_files {
        ...
        assert!(file_path.exists(), "UI file {} should exist in new crate", file);
    }
...
fn test_editor_app_extraction_structure() {
    ...
    assert!(main_path.exists(), "Main app file should be in engine-editor-app");
    
    let world_setup_path = app_path.join("src/world_setup.rs");
    assert!(world_setup_path.exists(), "World setup should be in app crate");
}
...
fn test_original_files_removed() {
    ...
    for file in &original_panels {
        ...
        assert!(!file_path.exists(), "Panel file {} should be removed from original location", file);
    }
    ...
    for file in &original_ui {
        let file_path = Path::new(file);
        assert!(!file_path.exists(), "UI file {} should be removed from original location", file);
    }
}
...
fn test_crate_dependencies() {
    ...
    if panels_cargo.exists() {
        ...
        assert!(content.contains("egui"), "Panels crate should depend on egui");
        assert!(content.contains("engine-ecs-core"), "Panels crate should depend on ECS");
        assert!(content.contains("engine-components-3d"), "Panels crate should depend on components");
    }
    ...
    if ui_cargo.exists() {
        ...
        assert!(content.contains("eframe"), "UI crate should depend on eframe");
    }
    ...
    if app_cargo.exists() {
        ...
        assert!(content.contains("engine-editor-panels"), "App should depend on panels crate");
        assert!(content.contains("engine-editor-ui"), "App should depend on UI crate");
        assert!(content.contains("engine-editor-scene-view"), "App should depend on scene view");
    }
}
...
fn test_interface_definitions() {
    ...
    if panels_lib.exists() {
        ...
        assert!(content.contains("pub trait Panel"), "Should define Panel trait");
        assert!(content.contains("pub use inspector::InspectorPanel"), "Should export InspectorPanel");
        assert!(content.contains("pub use hierarchy::HierarchyPanel"), "Should export HierarchyPanel");
    }
    ...
    if ui_lib.exists() {
        ...
        assert!(content.contains("pub use toolbar::Toolbar"), "Should export Toolbar");
        assert!(content.contains("pub use menu_bar::MenuBar"), "Should export MenuBar");
        assert!(content.contains("pub mod styling"), "Should export styling module");
    }
}
    ...
    if cargo_toml.exists() {
        ...
        if !content.contains("[[bin]]") {
            ...
            if app_cargo.exists() {
                ...
                assert!(app_content.contains("[[bin]]"), "Binary should be in app crate");
                assert!(app_content.contains("longhorn-editor"), "Should still be named longhorn-editor");
            }
        }
    }
}