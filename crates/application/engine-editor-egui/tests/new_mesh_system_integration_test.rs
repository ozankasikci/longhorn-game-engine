...
use engine_components_3d::{Transform, MeshFilter, MeshRenderer, Material, Visibility};
...
fn test_entities_render_with_new_mesh_system() {
    ...
    world.add_component(cube, MeshFilter::new(cube_mesh_handle)).unwrap();
    ...
    world.add_component(sphere, MeshFilter::new(sphere_mesh_handle)).unwrap();
    
    // Add MeshRenderer with shadows disabled
    world.add_component(sphere, MeshRenderer {
        cast_shadows: false,
    ...
    world.add_component(sphere, Name::new("Green Sphere")).unwrap();
        ...
        if let Some(mesh_filter) = world.get_component::<MeshFilter>(entity) {
            if let Some(mesh_renderer) = world.get_component::<MeshRenderer>(entity) {
                if mesh_renderer.enabled {
                    ...
                    assert!(world.get_component::<Material>(entity).is_some(), "Material component missing");
                    assert!(world.get_component::<Visibility>(entity).is_some(), "Visibility component missing");
...
fn test_disabled_entities_not_rendered() {
    ...
    world.add_component(enabled, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(1)))).unwrap();
    world.add_component(enabled, MeshRenderer {
        enabled: true,
        ..Default::default()
    }).unwrap();
    ...
    world.add_component(disabled, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(2)))).unwrap();
    world.add_component(disabled, MeshRenderer {
        enabled: false,
        ..Default::default()
    }).unwrap();
    ...
    world.add_component(invisible, Transform::default()).unwrap();
    world.add_component(invisible, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(3)))).unwrap();
    world.add_component(invisible, MeshRenderer::default()).unwrap();
    world.add_component(invisible, Visibility { visible: false }).unwrap();
        ...
        if let Some(_mesh_filter) = world.get_component::<MeshFilter>(entity) {
            if let Some(mesh_renderer) = world.get_component::<MeshRenderer>(entity) {
                if mesh_renderer.enabled {
                    ...
                    let is_visible = world.get_component::<Visibility>(entity)
                        .map(|v| v.visible)
...
fn test_layer_mask_filtering() {
    ...
    world.add_component(default_entity, Transform::default()).unwrap();
    world.add_component(default_entity, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(1)))).unwrap();
    world.add_component(default_entity, MeshRenderer {
    ...
    world.add_component(ui_entity, Transform::default()).unwrap();
    world.add_component(ui_entity, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(2)))).unwrap();
    world.add_component(ui_entity, MeshRenderer {
    ...
    world.add_component(water_entity, Transform::default()).unwrap();
    world.add_component(water_entity, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(3)))).unwrap();
    world.add_component(water_entity, MeshRenderer {
    ...
    assert_eq!(visible_count, 2, "Should render 2 entities (default and water layers)");
}