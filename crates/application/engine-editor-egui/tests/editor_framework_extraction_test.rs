...
fn test_framework_crate_exists() {
    ...
    assert!(
        framework_crate_path.exists(),
        "engine-editor-framework crate should exist"
    );
}
...
fn test_framework_crate_dependencies() {
    ...
    if cargo_path.exists() {
        ...
        assert!(
            contents.contains("engine-ecs-core"),
            "Should depend on ECS core"
        );
        assert!(
            contents.contains("engine-components-3d"),
            "Should depend on 3D components"
        );
        assert!(
            contents.contains("engine-editor-assets"),
            "Should depend on asset crate"
        );
        assert!(
            contents.contains("engine-editor-scene-view"),
            "Should depend on scene view"
        );
    }
}
...
fn test_console_message_system() {
    ...
    match &info_msg {
        ConsoleMessage::Message {
            ...
            assert!(matches!(
                message_type,
                engine_editor_framework::ConsoleMessageType::Info
            ));
        }
...
fn test_world_setup_functionality() {
    ...
    assert!(world
        .get_component::<engine_components_ui::Name>(cube_entity)
        .is_some());

    // Check for transform component
    let has_transform = world
        .get_component::<engine_components_3d::Transform>(cube_entity)
        .is_some();
...
fn test_play_state_management() {
    ...
    use engine_editor_framework::{PlayState, PlayStateManager};
...
fn test_hierarchy_object_types() {
    ...
    let parent_obj = HierarchyObject::parent(
        "Parent",
        vec![
...
fn test_editor_integration() {
    ...
    assert!(
        contents.contains("engine-editor-framework"),
