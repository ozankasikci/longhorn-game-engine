...
impl MockSceneRenderer {
    ...
    fn collect_renderable_entities(world: &World) -> Vec<(engine_ecs_core::Entity, [f32; 3], ResourceId, [f32; 4])> {
            ...
            if let Some(mesh_filter) = world.get_component::<MeshFilter>(entity) {
                ...
                if let Some(mesh_renderer) = world.get_component::<MeshRenderer>(entity) {
                    ...
                    if mesh_renderer.enabled {
                        ...
                        let material = world.get_component::<Material>(entity)
                            .cloned()
                            .unwrap_or_default();
                        
                        renderable_entities.push((entity, transform.position, mesh_filter.mesh.id(), material.color));
                    }
                }
            }
        }
...
fn test_scene_renderer_finds_new_mesh_entities() {
    ...
    world.add_component(cube1, Transform {
        position: [0.0, 0.0, 0.0],
        rotation: [0.0, 0.0, 0.0],
        scale: [1.0, 1.0, 1.0],
    }).unwrap();
    world.add_component(cube1, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(1)))).unwrap();
    world.add_component(cube1, MeshRenderer::default()).unwrap();
    world.add_component(cube1, Material {
    ...
    world.add_component(cube2, Transform {
        position: [2.0, 0.0, 0.0],
        rotation: [0.0, 45.0, 0.0],
        scale: [0.5, 0.5, 0.5],
    }).unwrap();
    world.add_component(cube2, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(2)))).unwrap();
    world.add_component(cube2, MeshRenderer::default()).unwrap();
    world.add_component(cube2, Material {
    ...
    world.add_component(disabled_cube, Transform::default()).unwrap();
    world.add_component(disabled_cube, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(3)))).unwrap();
    world.add_component(disabled_cube, MeshRenderer {
        enabled: false,
        ..Default::default()
    }).unwrap();
    world.add_component(disabled_cube, Material::default()).unwrap();
    world.add_component(disabled_cube, Name::new("Disabled Cube")).unwrap();
...
fn test_scene_renderer_handles_missing_components() {
    ...
    world.add_component(complete_entity, Transform::default()).unwrap();
    world.add_component(complete_entity, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(1)))).unwrap();
    world.add_component(complete_entity, MeshRenderer::default()).unwrap();
    
    // Entity missing MeshRenderer (should not render)
    let no_renderer = world.spawn();
    world.add_component(no_renderer, Transform::default()).unwrap();
    world.add_component(no_renderer, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(2)))).unwrap();
    
    // Entity missing MeshFilter (should not render)
    let no_filter = world.spawn();
    world.add_component(no_filter, Transform::default()).unwrap();
    world.add_component(no_filter, MeshRenderer::default()).unwrap();
    
    // Entity with only Transform (should not render)
    let transform_only = world.spawn();
    world.add_component(transform_only, Transform::default()).unwrap();
    
    // Collect renderable entities
    let renderable = MockSceneRenderer::collect_renderable_entities(&world);
    
    assert_eq!(renderable.len(), 1, "Should find only 1 complete entity");
    assert_eq!(renderable[0].0, complete_entity, "Should be the complete entity");
}
...
fn test_scene_renderer_respects_layer_mask() {
    ...
    world.add_component(default_layer, Transform::default()).unwrap();
    world.add_component(default_layer, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(1)))).unwrap();
    world.add_component(default_layer, MeshRenderer {
    ...
    world.add_component(ui_layer, MeshFilter::new(ResourceHandle::<MeshData>::new(ResourceId::new(2)))).unwrap();
    world.add_component(ui_layer, MeshRenderer {
    ...
    assert_eq!(default_layer_entities.len(), 1, "Should have 1 entity on default layer");
    assert_eq!(ui_layer_entities.len(), 1, "Should have 1 entity on UI layer");
}