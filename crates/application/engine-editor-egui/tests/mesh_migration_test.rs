...
fn test_query_entities_with_new_mesh_components() {
    ...
    assert_eq!(
        count, 1,
        "Should find exactly one entity with new mesh components"
    );
}
...
fn test_old_mesh_queries_return_empty_with_new_components() {
    ...
    assert_eq!(
        old_mesh_count, 0,
        "Should find no entities with old Mesh component"
    );
    ...
    assert_eq!(
        new_mesh_count, 1,
        "Should find entity with new mesh components"
    );
}
...
fn test_multiple_entities_with_different_mesh_types() {
    ...
    world
        ...
        .unwrap();
    world
        .add_component(sphere, MeshRenderer::default())
        .unwrap();
    world
        .add_component(
            sphere,
...
fn test_renderer_query_pattern() {
    ...
    assert_eq!(
        ...
        1,
        "Only one entity should be renderable"
    );
    assert_eq!(
        renderable_entities[0], e1,
        "Entity 1 should be the renderable one"
    );
}
...
fn test_mesh_resource_handles() {
    ...
    world
        .add_component(e1, MeshFilter::new(handle1.clone()))
        .unwrap();

    let e2 = world.spawn();
    let handle2 = ResourceHandle::<MeshData>::new(ResourceId::new(200));
    world
        .add_component(e2, MeshFilter::new(handle2.clone()))
        .unwrap();
    ...
    assert_ne!(
        filter1.mesh.id(),
        filter2.mesh.id(),
        "Mesh handles should be different"
    );
