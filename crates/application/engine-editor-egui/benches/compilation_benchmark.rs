...
fn benchmark_crate_sizes(c: &mut Criterion) {
    c.bench_function("measure_crate_line_counts", |b| {
        b.iter(|| {
            ...
            assert!(reduction > 90.0, "Should have >90% reduction in main crate size");
        });
    });
}
...
criterion_group!(benches, benchmark_crate_sizes, benchmark_module_independence, benchmark_parallel_compilation);
