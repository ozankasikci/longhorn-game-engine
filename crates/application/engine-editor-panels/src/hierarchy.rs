...
use crate::types::{ConsoleMessage, HierarchyObject, SceneTool};
use eframe::egui;
...
impl HierarchyPanel {
    ...
    pub fn show(
        ...
        gizmo_system: &mut dyn crate::types::GizmoSystem,
    ) -> Vec<ConsoleMessage> {
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ui.menu_button("+", |ui| {
                    ...
                    if ui.button("Empty GameObject").clicked() {
                        ...
                        world
                            .add_component(entity, Name::new("GameObject"))
                            .unwrap();
                        ui.close_menu();
                    }
                });
        ...
        egui::ScrollArea::vertical().show(ui, |ui| {
            ...
            for (entity, _transform) in world.query_legacy::<Transform>() {
                ...
                if world.get_component::<Visibility>(entity).is_some() {
                    components.push("V");
                }
                if world.get_component::<Camera>(entity).is_some() {
                    components.push("C");
                }
                if world.get_component::<Light>(entity).is_some() {
                    components.push("L");
                }
                if world.get_component::<SpriteRenderer>(entity).is_some() {
                    components.push("Spr");
                }
                ...
                if world.get_component::<Material>(entity).is_some() {
                    components.push("Mat");
                }

                let component_str = if components.is_empty() {
                    "-".to_string()
                } else {
                    components.join("")
                };
    ...
    pub fn show_hierarchy_objects(
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                if ui
                    .button("⚙️")
                    .on_hover_text("Hierarchy settings")
                    .clicked()
                {
                    // Settings clicked
                }
...
fn create_cube_entity(world: &mut World) -> Entity {
    ...
    world
        .add_component(entity, MeshFilter::new(mesh_handle))
        .unwrap();

    // Add MeshRenderer component with default material
    world
        .add_component(entity, MeshRenderer::default())
        .unwrap();
    ...
    world
        .add_component(entity, Name::new(format!("Cube {}", world.entity_count())))
        .unwrap();
...
fn create_sphere_entity(world: &mut World) -> Entity {
    ...
    world
        .add_component(
            entity,
            Name::new(format!("Sphere {}", world.entity_count())),
        )
        .unwrap();
...
fn create_plane_entity(world: &mut World) -> Entity {
    ...
    world.add_component(entity, Visibility::default()).unwrap();
    world
        .add_component(entity, Name::new("Ground Plane"))
        .unwrap();
