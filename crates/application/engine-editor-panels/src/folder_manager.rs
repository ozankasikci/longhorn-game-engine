...
impl std::fmt::Display for FolderOperationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FolderOperationError::AlreadyExists(name) => {
                write!(f, "Folder already exists: {}", name)
            }
...
impl FolderManager {
    ...
    pub fn create_folder(
        ...
        parent_path: &Path,
        folder_name: &str,
    ) -> Result<(), FolderOperationError> {
        // Validate folder name
        if folder_name.is_empty() {
            return Err(FolderOperationError::InvalidName(
    ...
    pub fn delete_folder(&self, folder_path: &Path) -> Result<(), FolderOperationError> {
        ...
        if !full_path.exists() {
            return Err(FolderOperationError::InvalidPath(
    ...
    pub fn rename_folder(
        ...
        let parent = full_old_path.parent().ok_or_else(|| {
            FolderOperationError::InvalidPath("Cannot rename root folder".to_string())
        })?;
    ...
    pub fn move_folder(
        ...
        let folder_name = full_source.file_name().ok_or_else(|| {
            FolderOperationError::InvalidPath("Cannot get folder name".to_string())
        })?;
    ...
    pub fn load_project_structure(
        &self,
        path: &Path,
    ) -> Result<Vec<ProjectAsset>, FolderOperationError> {
    ...
    fn load_directory(&self, dir_path: &Path) -> Result<Vec<ProjectAsset>, FolderOperationError> {
        ...
        assets.sort_by(|a, b| match (&a.children, &b.children) {
...
mod tests {
    ...
    fn test_create_folder() {
        ...
        assert!(matches!(
            result,
            Err(FolderOperationError::AlreadyExists(_))
        ));
    }
    ...
    fn test_rename_folder() {
        ...
        // Rename it
        manager
            .rename_folder(Path::new("old_name"), "new_name")
            .unwrap();
    ...
    fn test_move_folder() {
        ...
        manager
            .move_folder(Path::new("source"), Path::new("target"))
            .unwrap();
    ...
    fn test_move_file() {
        ...
        manager
            .move_file(Path::new("test.txt"), Path::new("target"))
            .unwrap();
