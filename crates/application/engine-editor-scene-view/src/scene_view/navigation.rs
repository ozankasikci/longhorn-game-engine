...
impl SceneNavigator {
    ...
    pub fn apply_mouse_look(scene_nav: &mut SceneNavigation, mouse_delta: egui::Vec2) {
        ...
        scene_nav.scene_camera_transform.rotation[0] =
            scene_nav.scene_camera_transform.rotation[0].clamp(-1.5, 1.5); // ~85 degrees
    ...
    pub fn handle_wasd_movement(scene_nav: &mut SceneNavigation, ui: &egui::Ui, delta_time: f32) {
        ...
        if movement_magnitude > 0.001 {
            ...
            let transformed_movement = super::camera_movement::transform_movement_by_camera(
                &scene_nav.scene_camera_transform,
                movement,
            );
    ...
    pub fn handle_navigation_speed_control(scene_nav: &mut SceneNavigation, ui: &egui::Ui) {
        ...
        if scroll_delta.abs() > 0.1 {
            ...
            scene_nav.movement_speed =
                (scene_nav.movement_speed * speed_multiplier).clamp(0.5, 50.0);
        }
        ...
        if scene_navigation.is_navigating && is_dragging {
            ...
            if let Some(current_pos) = pointer_pos {
                let effective_delta = if let Some(last_pos) = scene_navigation.last_mouse_pos {
                    ...
                    let manual_delta =
                        egui::Vec2::new(current_pos.x - last_pos.x, current_pos.y - last_pos.y);

                    // Filter out tiny movements to reduce jitter
                    // Only process movement if it's above a small threshold
                    if manual_delta.length() > 0.2 {
                        // Small threshold to filter micro-movements
                        manual_delta
                    } else {
                        egui::Vec2::ZERO
                    }
                ...
                if effective_delta.length() > 0.0 {
                    // Process any non-zero delta since we already filtered
...
mod tests {
    ...
    fn test_w_key_moves_camera_in_look_direction() {
        ...
        assert!(
            transformed[2] > 0.0,
            "W key with no rotation should move +Z"
        );
        ...
        let transformed_180 =
            SceneNavigator::transform_movement_by_camera(&camera_transform_180, movement);

        // When rotated 180, W should move us in -Z direction
        assert!(
            transformed_180[2] < 0.0,
            "W key when rotated 180 should move -Z"
        );
    }
    ...
    fn test_forward_movement_with_yaw_rotation() {
        ...
        assert!(
            transformed[0] > 0.9,
            "Forward movement should be along +X when rotated right"
        );
    ...
    fn test_forward_movement_with_pitch_rotation() {
        ...
        assert!(
            transformed[2] < 0.0,
            "Should still move forward (negative Z)"
        );
    ...
    fn test_backward_movement_is_opposite_of_forward() {
        ...
        let forward_transformed =
            SceneNavigator::transform_movement_by_camera(&camera_transform, forward_movement);
        let backward_transformed =
            SceneNavigator::transform_movement_by_camera(&camera_transform, backward_movement);
