...
impl SceneViewRenderer {
    ...
    pub fn initialize_renderer(
        ...
        let mut game_renderer =
            pollster::block_on(async { Renderer3D::new(device, queue, 800, 600).await })?;
    ...
    pub fn draw_scene(
        ...
        self.editor_camera
            .handle_input(world, ui, response, delta_time);

        // Sync camera with scene navigation (for compatibility)
        self.editor_camera
            .sync_to_scene_navigation(world, scene_navigation);
            ...
            (&mut self.render_widget, &self.ecs_bridge)
        {
            ...
            if let Some(entity) = selected_entity {
                if let Some(transform) = world.get_component::<Transform>(entity) {
                    ...
                    log::info!(
                        "3D Gizmo enabled for entity at position {:?}",
                        transform.position
                    );
                } else {
            ...
            let render_scene = self.ecs_bridge.as_ref().unwrap().world_to_render_scene(
                world,
    ...
    pub fn render_game_camera_view(
        ...
        log::info!(
            "render_game_camera_view called with camera pos: {:?}, target: {:?}",
            camera.position,
            camera.target
        );
        ...
        if let (Some(render_widget), Some(ecs_bridge)) =
            (&mut self.game_render_widget, &self.ecs_bridge)
        {
            ...
            log::info!(
                "Created game view render scene with {} objects",
                render_scene.objects.len()
            );
