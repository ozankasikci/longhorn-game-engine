    ...
    pub fn analyze(&self, audio_data: &AudioData) -> Result<AudioAnalysis, AudioError> {
        match audio_data.format {
            AudioFormat::PcmF32 => {
                ...
                let peak_amplitude = samples_f32.iter()
                    .map(|s| s.abs())
                    .fold(0.0f32, |a, b| a.max(b));
                
                // Calculate RMS
                let sum_squares: f32 = samples_f32.iter()
                    .map(|s| s * s)
                    .sum();
                let rms_level = (sum_squares / samples_f32.len() as f32).sqrt();
                
                // Check for clipping
                let is_clipping = samples_f32.iter()
                    .any(|&s| s.abs() >= 1.0);
                ...
                let dominant_frequency = (zero_crossings as f32 / 2.0) / audio_data.duration_seconds as f32;
            ...
            _ => Err(AudioError::ProcessingError("Analysis only supported for f32 format".to_string())),
        }
    }
}