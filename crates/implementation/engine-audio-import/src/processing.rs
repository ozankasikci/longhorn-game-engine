use crate::{AudioData, AudioError, AudioFormat, ChannelLayout, SampleRate};
use rubato::{
...
impl AudioNormalizer {
    ...
    pub fn normalize(
        ...
        match audio_data.format {
            AudioFormat::PcmF32 => {
                let mut samples_f32: Vec<f32> = audio_data
                    .samples
                    .chunks(4)
                    .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
                    .collect();
                ...
                audio_data.samples = samples_f32.iter().flat_map(|s| s.to_le_bytes()).collect();
            }
            _ => {
                return Err(AudioError::ProcessingError(
                    "Normalization only supported for f32 format".to_string(),
                ))
            }
        }
        ...
        match audio_data.format {
            AudioFormat::PcmS16 => {
                ...
                audio_data.duration_seconds =
                    trimmed_samples.len() as f64 / sample_rate as f64 / channels as f64;
            }
            _ => {
                return Err(AudioError::ProcessingError(
                    "Trimming only supported for S16 format".to_string(),
                ))
            }
        }
