...
fn test_topology_validation() {
    ...
    use engine_mesh_import::validation::{TopologyError, TopologyValidator};
...
fn test_uv_validation() {
    ...
    use engine_mesh_import::validation::{UVError, UVValidator};
    ...
    assert!(warnings
        .iter()
        .any(|w| matches!(w, UVError::OutOfRange(..))));
    ...
    assert!(warnings
        .iter()
        .any(|w| matches!(w, UVError::OverlappingFaces(..))));
}

#[test]
fn test_mesh_optimization_pipeline() {
    // Test 3: Verify comprehensive mesh optimization pipeline
    use engine_mesh_import::optimization::{OptimizationOptions, OptimizationPipeline};
...
fn test_lod_generation() {
    ...
    use engine_mesh_import::lod::{LODGenerator, LODLevel, LODOptions};
    ...
    let triangle_counts: Vec<usize> = lod_meshes
        .iter()
...
fn test_uv_unwrapping() {
    ...
    let uv_set: std::collections::HashSet<_> = mesh_no_uvs
        .vertices
        .iter()
        .map(|v| {
            (
...
fn test_advanced_normal_generation() {
    ...
    use engine_mesh_import::processing::{NormalOptions, NormalProcessor, SmoothingMethod};
    ...
    let options = NormalOptions {
        method: SmoothingMethod::AngleBased {
    ...
    for vertex in &mesh.vertices {
        let length =
            (vertex.normal[0].powi(2) + vertex.normal[1].powi(2) + vertex.normal[2].powi(2)).sqrt();
        assert!((length - 1.0).abs() < 0.001);
    }

    // Verify normals were generated (not all zero)
    let has_non_zero_normals = mesh
        .vertices
        .iter()
...
fn test_vertex_cache_optimization() {
    ...
    use engine_mesh_import::optimization::{CacheOptions, VertexCacheOptimizer};
...
fn test_batch_processing() {
    ...
    use engine_mesh_import::processing::{BatchOptions, BatchProcessor};
