...
impl TopologyValidator {
    ...
    fn check_open_edges(&self, mesh: &MeshData) -> Result<(), TopologyError> {
        ...
        for face in mesh.indices.chunks(3) {
            ...
            edge_count.entry((face[0], face[1])).and_modify(|c| *c += 1).or_insert(1);
            edge_count.entry((face[1], face[2])).and_modify(|c| *c += 1).or_insert(1);
            edge_count.entry((face[2], face[0])).and_modify(|c| *c += 1).or_insert(1);
            
            // Subtract reverse edges
            edge_count.entry((face[1], face[0])).and_modify(|c| *c -= 1).or_insert(-1);
            edge_count.entry((face[2], face[1])).and_modify(|c| *c -= 1).or_insert(-1);
            edge_count.entry((face[0], face[2])).and_modify(|c| *c -= 1).or_insert(-1);
        }
