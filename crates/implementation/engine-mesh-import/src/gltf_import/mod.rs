use crate::types::{MeshData, Vertex, Material, MaterialProperty, MeshImporter};
        ...
        for mesh in document.meshes() {
            ...
            for primitive in mesh.primitives() {
                ...
                let position_data = match positions.view() {
                    Some(view) => &buffers[view.buffer().index()][view.offset()..view.offset() + view.length()],
                    None => return Err(ImportError::ParseError("No buffer view for positions".into())),
                };
                
                // Get normals if available
                let normals = primitive.get(&gltf::Semantic::Normals);
                let normal_data = normals.and_then(|n| n.view())
                    .map(|view| &buffers[view.buffer().index()][view.offset()..view.offset() + view.length()]);
                
                // Get texture coordinates if available
                let tex_coords = primitive.get(&gltf::Semantic::TexCoords(0));
                let tex_coord_data = tex_coords.and_then(|t| t.view())
                    .map(|view| &buffers[view.buffer().index()][view.offset()..view.offset() + view.length()]);
                ...
                if let Some(accessor) = primitive.indices() {
                    let view = accessor.view()
                        .ok_or_else(|| ImportError::ParseError("No buffer view for indices".into()))?;
                    ...
                    match accessor.data_type() {
                        gltf::accessor::DataType::U16 => {
                            for i in 0..accessor.count() {
                                let offset = i * 2;
                                let index = u16::from_le_bytes([data[offset], data[offset + 1]]) as u32;
                                indices.push(index);
                            }
                        }
                ...
                let material = {
                    ...
                    // Alpha mode
                    material.set_property(MaterialProperty::AlphaMode(format!("{:?}", mat.alpha_mode())));
                    material.set_property(MaterialProperty::AlphaCutoff(mat.alpha_cutoff().unwrap_or(0.5)));
