...
use engine_geometry_core::{Mesh, Vertex, VertexData, IndexBuffer, BoundingBox, mesh::PrimitiveTopology};
use glam::Vec3;
use thiserror::Error;
...
impl MeshConverter {
    ...
    pub fn convert(&self, import_mesh: &ImportMeshData) -> Result<Mesh, ConversionError> {
        ...
        let vertices: Vec<Vertex> = import_mesh.vertices.iter()
            .map(|v| Vertex {
    ...
    pub fn convert_multiple(&self, meshes: &[ImportMeshData]) -> Result<Vec<Mesh>, ConversionError> {
        meshes.iter()
