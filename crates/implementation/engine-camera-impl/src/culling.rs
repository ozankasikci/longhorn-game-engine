...
impl FrustumCullerFactory for DefaultFrustumCullerFactory {
    fn create_from_matrices(
        view: Mat4,
        projection: Mat4,
    ) -> engine_camera_core::Result<Box<dyn FrustumCuller>> {
        let view_proj = projection * view;
        Self::create_from_view_projection(view_proj)
    }

    fn create_from_view_projection(
        view_proj: Mat4,
    ) -> engine_camera_core::Result<Box<dyn FrustumCuller>> {
...
mod tests {
    ...
    fn test_bounding_volume_aabb_creation() {
        ...
        if let BoundingVolume::AABB { min, max } =
            BoundingVolume::aabb_from_center_size(center, size)
        {
