...
use super::{CameraController, CameraInput, ControllerConfig, helpers};
...
impl CameraController for FPSCameraController {
    fn update(
        ...
        if self.rotation_smoothing > 0.0 {
            ...
            let smoothed_rot = current_rot.lerp(self.euler_angles, 1.0 - self.rotation_smoothing.powf(delta_time));
...
mod tests {
    ...
    fn test_movement_transform() {
        ...
        assert!(transform.position[0] < -4.9, "Expected X < -4.9, got {}", transform.position[0]);
        assert!(transform.position[2].abs() < 0.1, "Expected minimal Z movement, got {}", transform.position[2]);
    }
}