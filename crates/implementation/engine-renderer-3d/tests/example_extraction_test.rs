...
fn test_examples_removed() {
    ...
    assert!(!examples_path.exists(), "Examples directory should be removed after extraction");
}
...
fn test_cargo_toml_cleaned() {
    ...
    assert!(!cargo_content.contains("[[example]]"), 
...
fn test_examples_removed() {
    ...
    let examples_path = Path::new("examples");
    assert!(!examples_path.exists(), "Examples should be moved to separate crate");
}
...
fn test_cargo_toml_updated() {
    ...
    let cargo_content = std::fs::read_to_string("Cargo.toml").unwrap();
    assert!(!cargo_content.contains("[[example]]"), "Example entries should be removed");
}