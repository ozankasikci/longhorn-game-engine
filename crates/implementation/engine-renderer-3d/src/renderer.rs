...
use crate::gizmo_3d::{GizmoMode, GizmoRenderer3D};
use crate::grid::{GridConfig, GridRenderer};
...
impl Renderer3D {
    ...
    pub async fn new(
        ...
        let indices: Vec<u16> = vec![
            ...
            0, 1, 2, 0, 2, 3, // Back face (CCW when viewed from back)
            4, 5, 6, 4, 6, 7, // Top face (CCW when viewed from top)
            8, 9, 10, 8, 10, 11, // Bottom face (CCW when viewed from bottom)
            12, 13, 14, 12, 14, 15, // Right face (CCW when viewed from right)
            16, 17, 18, 16, 18, 19, // Left face (CCW when viewed from left)
            20, 21, 22, 20, 22, 23,
        ];
        ...
        let default_materials =
            resource_manager.load_default_materials(&material_bind_group_layout)?;
        ...
        log::info!(
            "Loaded {} default meshes, {} default materials, {} default textures",
            default_meshes.len(),
            default_materials.len(),
            default_textures.len()
        );
    ...
    pub fn render(&mut self, scene: &RenderScene) -> Result<(), anyhow::Error> {
        ...
        // Begin render pass
        {
            ...
            if !scene.objects.is_empty() {
                for (idx, render_object) in scene.objects.iter().enumerate() {
                    log::info!(
                        "Drawing object {} with mesh_id: {}, material_id: {}",
                        idx,
                        render_object.mesh_id,
                    ...
                    log::info!(
                        "Drawing {} indices (should be 36 for a cube)",
                        self.num_indices
                    );
            ...
            if self.gizmo_enabled {
                if let (Some(gizmo), Some(transform)) =
                    (&self.gizmo_renderer, &self.gizmo_transform)
                {
                    gizmo.render(
                        &mut render_pass,
                        transform,
    ...
    pub fn upload_material(&self, material: Material) -> Result<u32, anyhow::Error> {
        self.resource_manager
            .upload_material(material, &self.material_bind_group_layout)
    }
    ...
    pub fn create_solid_color_texture(
        ...
        self.resource_manager
            .create_solid_color_texture(color, size)
    }
