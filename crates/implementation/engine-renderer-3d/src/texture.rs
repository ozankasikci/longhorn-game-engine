...
impl TextureManager {
    ...
    pub fn create_texture(&self, desc: TextureDescriptor) -> Result<u32, anyhow::Error> {
        ...
        let texture_resource = TextureResource {
            ...
            name: desc
                .label
                .unwrap_or_else(|| format!("Texture {}", texture_id)),
        };
        ...
        self.textures
            .write()
            .unwrap()
            .insert(texture_id, texture_resource);

        log::info!(
            "Created texture '{}' with ID {} ({}x{})",
            label,
            texture_id,
            desc.width,
            desc.height
    ...
    pub fn get_texture(
        &self,
        texture_id: u32,
    ) -> Option<std::sync::RwLockReadGuard<HashMap<u32, TextureResource>>> {
