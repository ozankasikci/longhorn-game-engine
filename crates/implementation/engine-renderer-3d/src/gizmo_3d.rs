...
impl GizmoRenderer3D {
    ...
    fn render_translation_gizmo<'a>(&'a self, render_pass: &mut wgpu::RenderPass<'a>) {
        eprintln!(
            "3D GIZMO: Rendering translation gizmo with 3 colored arrows and 3 plane handles"
        );
        ...
        render_pass.set_index_buffer(
            self.plane_xy_mesh.index_buffer.slice(..),
        ...
        render_pass.set_index_buffer(
            self.plane_xz_mesh.index_buffer.slice(..),
            wgpu::IndexFormat::Uint16,
        );
        ...
        render_pass.set_index_buffer(
            self.plane_yz_mesh.index_buffer.slice(..),
            wgpu::IndexFormat::Uint16,
        );
    ...
    fn render_circle<'a>(
        ...
        render_pass.set_index_buffer(
            self.circle_mesh.index_buffer.slice(..),
            wgpu::IndexFormat::Uint16,
        );
...
fn create_arrow_mesh_with_color(device: &wgpu::Device, color: [f32; 4]) -> GizmoMesh {
    ...
    vertices.push(GizmoVertex {
        position: [0.0, shaft_length + head_length, 0.0],
        color,
    });
    ...
    log::info!(
        "Created arrow mesh with {} vertices, {} indices, color: {:?}",
        vertices.len(),
        indices.len(),
        color
    );
...
fn create_plane_mesh_xy(device: &wgpu::Device) -> GizmoMesh {
    ...
    let vertices = vec![
        ...
        GizmoVertex {
            position: [offset + size, offset, 0.0],
            color,
        },
        GizmoVertex {
            position: [offset + size, offset + size, 0.0],
            color,
        },
        GizmoVertex {
            position: [offset, offset + size, 0.0],
            color,
        },
    ];
...
fn create_plane_mesh_xz(device: &wgpu::Device) -> GizmoMesh {
    ...
    let vertices = vec![
        ...
        GizmoVertex {
            position: [offset + size, 0.0, offset],
            color,
        },
        GizmoVertex {
            position: [offset + size, 0.0, offset + size],
            color,
        },
        GizmoVertex {
            position: [offset, 0.0, offset + size],
            color,
        },
    ];
...
fn create_plane_mesh_yz(device: &wgpu::Device) -> GizmoMesh {
    ...
    let vertices = vec![
        GizmoVertex {
            position: [0.0, offset, offset],
            color,
        },
        GizmoVertex {
            position: [0.0, offset + size, offset],
            color,
        },
        GizmoVertex {
            position: [0.0, offset + size, offset + size],
            color,
        },
        GizmoVertex {
            position: [0.0, offset, offset + size],
            color,
        },
    ];
