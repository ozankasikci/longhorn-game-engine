...
use crate::{RenderScene, Renderer3D};
use egui::epaint::ImageDelta;
use egui::{ColorImage, Response, TextureId, TextureOptions, Ui, Widget};
...
impl Widget for &mut EguiRenderWidget {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        log::info!(
            "EguiRenderWidget::ui called with available_rect: {:?}, size: {:?}",
            available_rect,
            size
        );
        ...
        if let Some(texture_id) = self.texture_id {
            log::info!(
                "Drawing texture with id {:?} at rect {:?}",
                texture_id,
                rect
            );
...
pub fn create_render_widget(
    ...
    let renderer =
        pollster::block_on(async { Renderer3D::new(device, queue, width, height).await })?;
