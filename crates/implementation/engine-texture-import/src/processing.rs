use crate::{TextureData, TextureFormat, TextureError, MipmapLevel};
...
impl TextureResizer {
    ...
    pub fn resize(&self, texture: &mut TextureData, options: &ResizeOptions) -> Result<(), TextureError> {
...
impl NormalMapProcessor {
    ...
    pub fn height_to_normal(&self, texture: &mut TextureData, options: &NormalMapOptions) -> Result<(), TextureError> {
        if texture.format != TextureFormat::R8 {
            return Err(TextureError::InvalidData("Expected R8 height map".to_string()));
        }
...
impl FormatConverter {
    ...
    pub fn convert(&self, texture: &TextureData, target_format: TextureFormat) -> Result<TextureData, TextureError> {
