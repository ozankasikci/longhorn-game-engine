...
use crate::{Event, EventTypeId, Result, EventError};
...
pub trait EventHandlerTrait: Send + Sync {
    ...
    fn priority(&self) -> HandlerPriority { HandlerPriority::Normal }
}
...
impl EventDispatcher {
    ...
    pub fn register_handler(&mut self, type_id: EventTypeId, handler: Box<dyn EventHandlerTrait>) -> Result<()> {
...
impl<T: Event + 'static> SimpleEventHandler<T> {
    ...
    pub fn with_priority(handler: EventHandler<T>, type_id: EventTypeId, priority: HandlerPriority) -> Self {
        Self {
            handler,
            priority,
            type_id,
