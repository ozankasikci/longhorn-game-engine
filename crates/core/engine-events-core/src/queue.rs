...
use crate::{Event, EventId, EventPriority, Result, EventError};
...
impl Ord for QueuedEvent {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        self.priority.cmp(&other.priority).reverse()
            .then_with(|| self.queued_at.partial_cmp(&other.queued_at).unwrap_or(Ordering::Equal))
    }
}
...
impl EventQueue {
    ...
    fn drop_oldest_event(&mut self) {
        ...
        if let Some(oldest_idx) = self.events.iter()
            .enumerate()
            .max_by(|a, b| a.1.queued_at.partial_cmp(&b.1.queued_at).unwrap_or(Ordering::Equal))
            .map(|(idx, _)| idx)
        {
