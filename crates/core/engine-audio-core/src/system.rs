...
use crate::{
    AudioSource, SpatialAudioSource, AudioListener, StreamingAudioSource, 
...
impl<T: AudioManager> AudioSystem for AudioSystemImpl<T> {
            ...
            if let Some(playbacks) = self.entity_playbacks.get(&entity) {
                for &playback_handle in playbacks {
                    if self.audio_manager.is_playback_active(playback_handle) {
                        self.audio_manager.set_playback_volume(playback_handle, audio_source.volume)?;
                        self.audio_manager.set_playback_pitch(playback_handle, audio_source.pitch)?;
                    }
                }
            }
        }
    ...
    fn update_spatial_audio(&mut self, world: &World) -> Result<()> {
        ...
        for (entity, spatial_source) in world.query::<SpatialAudioSource>() {
            ...
            if let Some(transform) = world.get_component::<Transform>(entity) {
                ...
                let directional_attenuation = spatial_source.calculate_directional_attenuation(direction_to_listener);
                ...
                if let Some(playbacks) = self.entity_playbacks.get(&entity) {
                    for &playback_handle in playbacks {
                        if self.audio_manager.is_playback_active(playback_handle) {
                            self.audio_manager.set_playback_volume(playback_handle, final_volume)?;
                        }
                    }
                }
            }
    ...
    fn play_entity_audio(&mut self, world: &World, entity: Entity) -> Result<()> {
        if let Some(audio_source) = world.get_component::<AudioSource>(entity) {
            ...
            // Track the playback
            self.entity_playbacks.entry(entity)
