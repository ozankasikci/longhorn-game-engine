...
pub fn calculate_mvp_matrix(model: &Mat4, view: &Mat4, projection: &Mat4) -> Mat4 {
...
mod tests {
    ...
    fn test_perspective_matrix() {
        ...
        assert!(
            near_z < far_z,
            "Near plane should have smaller Z than far plane"
        );
        assert!(
            near_z < mid_z && mid_z < far_z,
            "Z should increase with distance"
        );
    }

    #[test]
    fn test_orthographic_matrix() {
        let ortho = calculate_orthographic_matrix(-10.0, 10.0, -10.0, 10.0, -1.0, 1.0);
