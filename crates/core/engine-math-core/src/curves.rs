...
pub fn bspline_basis(t: f32, i: i32) -> f32 {
    match i {
        ...
        1 => (3.0 * t * t * t - 6.0 * t * t + 4.0) / 6.0,
        2 => (-3.0 * t * t * t + 3.0 * t * t + 3.0 * t + 1.0) / 6.0,
        3 => t * t * t / 6.0,
        _ => 0.0,
    }
}
...
fn estimate_bezier_length_to_t(
    p0: Vec2,
    p1: Vec2,
    p2: Vec2,
    p3: Vec2,
...
mod tests {
    ...
    fn test_bspline_basis() {
        ...
        let t = 0.5;
        let sum =
            bspline_basis(t, 0) + bspline_basis(t, 1) + bspline_basis(t, 2) + bspline_basis(t, 3);
