...
impl World {
                    ...
                    if let Some(swapped_entity) = archetype.entities().last().copied() {
                        if swapped_entity != entity {
                            if let Some(swapped_location) =
                                self.entity_locations.get_mut(&swapped_entity)
                            {
    ...
    fn clone_entity_components(
        ...
        for type_id in location.archetype_id.type_ids() {
            if let Some(component) = archetype.clone_component_at(*type_id, location.index)? {
                let ticks = archetype
                    .get_component_ticks_at(*type_id, location.index)
    ...
    fn migrate_entity_to_new_archetype<T: Component>(
        ...
        self.entity_locations.insert(
            entity,
            EntityLocation::new(target_archetype_id.clone(), new_index),
        );
    ...
    pub fn add_component<T: Component>(&mut self, entity: Entity, component: T) -> EcsResult<()> {
        ...
        if let Some(old_location) = self.entity_locations.get(&entity).cloned() {
            if old_location.archetype_id() == &target_archetype_id {
                ...
                return self.migrate_entity_to_new_archetype(
                    entity,
                    old_location,
                    target_archetype_id,
                    component,
        ...
        self.entity_locations
            .insert(entity, EntityLocation::new(target_archetype_id, index));
    ...
    pub fn remove_component<T: Component>(&mut self, entity: Entity) -> EcsResult<()> {
            ...
            for type_id in location.archetype_id().type_ids() {
                if *type_id != TypeId::of::<T>() {
                    if let Some(component) =
                        archetype.clone_component_at(*type_id, location.index())?
                    {
                        let ticks = archetype
                            .get_component_ticks_at(*type_id, location.index())
        ...
        if !components_to_keep.is_empty() {
            ...
            self.entity_locations
                .insert(entity, EntityLocation::new(new_archetype_id, index));
        }
    ...
    pub fn par_query<T: Component + Sync>(&self) -> impl ParallelIterator<Item = (Entity, &T)> {
        self.archetypes
            ...
            .flat_map(|(_, archetype)| {
                ...
                (0..entities.len()).into_par_iter().filter_map(move |idx| {
                    component_array
                        .and_then(|arr| arr.get(idx))
...
mod tests {
    ...
    fn test_query_legacy() {
        ...
        let e1 = world.spawn();
        world
            .add_component(e1, Position::new(1.0, 0.0, 0.0))
            .unwrap();

        let e2 = world.spawn();
        world
            .add_component(e2, Position::new(2.0, 0.0, 0.0))
            .unwrap();
        world
            .add_component(e2, Velocity::new(1.0, 1.0, 1.0))
            .unwrap();

        let e3 = world.spawn();
        world
            .add_component(e3, Position::new(3.0, 0.0, 0.0))
            .unwrap();

        let e4 = world.spawn();
        world
            .add_component(e4, Velocity::new(2.0, 2.0, 2.0))
            .unwrap();
    ...
    fn test_update_existing_component() {
        ...
        world
            .add_component(entity, Position::new(10.0, 20.0, 30.0))
            .unwrap();
