...
impl Archetype {
    ...
    pub fn add_component<T: Component>(&mut self, component: T, ticks: ComponentTicks) -> EcsResult<()> {
        ...
        if let Some(array) = self.components.get_mut(&type_id) {
            array.inner_mut().push_cloned(
                Box::new(component) as Box<dyn ComponentClone>,
                ticks
            )
        } else {
            // Create new component array
            if let Some(mut new_array) = create_component_array(type_id) {
                new_array.push_cloned(
                    Box::new(component) as Box<dyn ComponentClone>,
                    ticks
                )?;
                self.components.insert(type_id, ErasedComponentArray::new(new_array));
                Ok(())
            } else {
    ...
    pub fn clone_component_at(&self, type_id: TypeId, index: usize) -> EcsResult<Option<Box<dyn ComponentClone>>> {
        let array = self.components.get(&type_id)
        ...
        for &type_id in self.id.type_ids() {
            if !self.components.contains_key(&type_id) {
                if let Some(array) = create_component_array(type_id) {
                    self.components.insert(type_id, ErasedComponentArray::new(array));
                } else {
...
mod tests {
    use super::*;
    use crate::ecs_v2::{entity::Entity, component::{register_component, ComponentArrayTrait}};
    ...
    fn test_archetype_component_storage() {
        ...
        archetype.add_component(Position { x: 10.0, y: 20.0 }, tick).unwrap();
        archetype.add_component(Velocity { x: 1.0, y: 2.0 }, tick).unwrap();
    ...
    fn test_archetype_entity_removal() {
        ...
        archetype.add_component(Position { x: 1.0, y: 1.0 }, tick).unwrap();
        
        archetype.add_entity(e2);
        archetype.add_component(Position { x: 2.0, y: 2.0 }, tick).unwrap();
        
        archetype.add_entity(e3);
        archetype.add_component(Position { x: 3.0, y: 3.0 }, tick).unwrap();
    ...
    fn test_archetype_component_arrays() {
        ...
        for i in 0..5 {
            ...
            archetype.add_component(Position { x: i as f32, y: i as f32 * 2.0 }, tick).unwrap();
        }
    ...
    fn test_archetype_component_not_registered() {
        ...
        #[derive(Clone, Debug, PartialEq)]
        struct UnregisteredComponent { data: String }
