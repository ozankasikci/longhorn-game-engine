...
use engine_ecs_core::{register_component, Component, World};
...
fn test_add_component_to_existing_entity() {
    ...
    assert!(
        result.is_ok(),
        "Failed to add component to existing entity: {:?}",
        result
    );
...
fn test_add_multiple_components_sequentially() {
    ...
    world
        ...
        .unwrap();
    world
        .add_component(entity, Velocity { dx: 5.0, dy: 5.0 })
        .unwrap();
    world
        .add_component(
            entity,
...
fn test_component_data_preserved_during_migration() {
    ...
    let entity = world.spawn();
    world
        .add_component(entity, Position { x: 42.0, y: 84.0 })
        .unwrap();

    // Add another component (triggers migration)
    world
        .add_component(entity, Velocity { dx: -1.0, dy: -2.0 })
        .unwrap();
...
fn test_multiple_entities_with_different_archetypes() {
    ...
    world
        ...
        .unwrap();
    world
        .add_component(e2, Velocity { dx: 10.0, dy: 20.0 })
        .unwrap();

    // Entity 3: Position + Velocity + Health
    let e3 = world.spawn();
    world
        .add_component(e3, Position { x: 3.0, y: 3.0 })
        .unwrap();
    world
        .add_component(e3, Velocity { dx: 30.0, dy: 40.0 })
        .unwrap();
    world
        .add_component(
            e3,
...
fn test_query_after_migration() {
    ...
    let positions: Vec<_> = world.query_legacy::<Position>().map(|(_, p)| p.x).collect();
    assert_eq!(positions.len(), 5);

    // Query all Velocity components
    let velocities: Vec<_> = world
        .query_legacy::<Velocity>()
        .map(|(_, v)| v.dx)
        .collect();
...
fn test_remove_component() {
    ...
    world
        ...
        .unwrap();
    world
        .add_component(entity, Velocity { dx: 5.0, dy: 10.0 })
        .unwrap();
    world
        .add_component(
            entity,
