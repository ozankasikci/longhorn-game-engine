...
use engine_resource_core::{Resource, ResourceHandle, ResourceId};
use glam::{Vec2, Vec3};
...
fn create_test_mesh_data() -> MeshData {
    ...
    MeshData::new("test_triangle".to_string(), vertices, vec![0, 1, 2])
}
...
fn test_mesh_data_memory_usage() {
    ...
    let expected_submesh_size = std::mem::size_of::<engine_geometry_core::SubMesh>();
...
fn test_mesh_data_with_multiple_submeshes() {
    ...
    let mut mesh_data = MeshData::new("multi_submesh".to_string(), vertices, indices);
