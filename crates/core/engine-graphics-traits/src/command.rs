use crate::{
    Color, ComputePipeline, GraphicsBindGroup, GraphicsBuffer, GraphicsPipeline,
...
pub trait GraphicsCommandEncoder: Send + Sync {
    ...
    /// Begin a render pass
    fn begin_render_pass<'a>(&'a mut self, desc: &RenderPassDescriptor<'a>)
        -> Self::RenderPass<'a>;
...
mod tests {
    ...
    impl GraphicsPipeline for MockPipeline {
        fn layout(&self) -> &dyn crate::GraphicsPipelineLayout {
            unimplemented!()
        }
    }
    ...
    impl GraphicsBindGroup for MockBindGroup {
        fn layout(&self) -> &dyn crate::GraphicsBindGroupLayout {
            unimplemented!()
        }
    }
