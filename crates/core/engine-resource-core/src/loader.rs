...
use crate::{ResourceId, ResourceHandle};
use crate::metadata::ResourceMetadata;
use crate::state::{LoadingState, LoadingPriority};
use std::any::Any;
use std::sync::Arc;
use std::path::Path;
use std::io;
...
pub trait ResourceLoader: Send + Sync {
    ...
    fn load(&self, path: &Path, metadata: &ResourceMetadata) -> LoaderResult<Arc<dyn Any + Send + Sync>>;
    ...
    fn cache_state(&mut self) -> LoaderResult<()> { Ok(()) }
    
    /// Clear any cached state
    fn clear_cache(&mut self) -> LoaderResult<()> { Ok(()) }
}

/// Future type for async loading operations
pub type LoaderFuture = Box<dyn std::future::Future<Output = LoaderResult<Arc<dyn Any + Send + Sync>>> + Send + Unpin>;
...
mod tests {
    ...
    fn test_format_detection() {
        ...
        assert_eq!(FormatDetector::from_magic_bytes(&png_data), Some("png".to_string()));
        
        // Test JPEG magic bytes
        let jpg_data = [0xFF, 0xD8, 0xFF, 0xE0];
        assert_eq!(FormatDetector::from_magic_bytes(&jpg_data), Some("jpg".to_string()));
    ...
    fn test_compression_detection() {
        ...
        assert_eq!(FormatDetector::detect_compression(&gzip_data), Some("gzip".to_string()));
        
        // Test ZIP magic bytes
        let zip_data = [0x50, 0x4B, 0x03, 0x04];
        assert_eq!(FormatDetector::detect_compression(&zip_data), Some("zip".to_string()));
    }
    ...
    fn test_loading_context() {
        ...
        context.metadata.insert("test".to_string(), "value".to_string());
