...
impl ResourceType {
    ...
    pub fn from_extension(extension: &str) -> Option<ResourceType> {
        match extension.to_lowercase().as_str() {
            // Texture formats
            "png" | "jpg" | "jpeg" | "bmp" | "tga" | "dds" | "ktx" | "astc" => Some(ResourceType::Texture),
            
            // Audio formats
            "wav" | "mp3" | "ogg" | "flac" | "aac" | "m4a" => Some(ResourceType::Audio),
            
            // Model formats
            "obj" | "fbx" | "dae" | "gltf" | "glb" | "blend" | "3ds" | "ply" => Some(ResourceType::Model),
            
            // Script formats
            "lua" | "js" | "ts" | "py" | "cs" | "cpp" | "c" | "h" => Some(ResourceType::Script),
            
            // Shader formats
            "glsl" | "hlsl" | "wgsl" | "vert" | "frag" | "geom" | "comp" | "tesc" | "tese" => Some(ResourceType::Shader),
...
mod tests {
    ...
    fn test_resource_type_detection() {
        assert_eq!(ResourceType::from_extension("png"), Some(ResourceType::Texture));
        assert_eq!(ResourceType::from_extension("wav"), Some(ResourceType::Audio));
        assert_eq!(ResourceType::from_extension("obj"), Some(ResourceType::Model));
    ...
    fn test_resource_metadata() {
        ...
        assert_eq!(metadata.get_custom_field("width").unwrap().as_integer(), Some(512));
        assert_eq!(metadata.get_custom_field("height").unwrap().as_integer(), Some(256));
    }
