...
impl StructureMigrator {
    ...
    pub fn fix_crate(
        ...
        for error in &report.errors {
            match error {
                ...
                ValidationError::InconsistentStructure(msg)
                    if msg.contains("workspace inheritance") =>
                {
    ...
    fn fix_missing_directory(
        ...
        if self.dry_run {
            actions.push(MigrationAction::WouldCreateDir { path: dir_path });
        } else {
            std::fs::create_dir_all(&dir_path)?;
            actions.push(MigrationAction::CreatedDir { path: dir_path });
        }

        Ok(())
    }

    fn fix_missing_cargo_field(
        &self,
        crate_path: &Path,
        field: &str,
        actions: &mut Vec<MigrationAction>,
    ...
    fn fix_workspace_inheritance(
        ...
        if self.dry_run {
            ...
            actions.push(MigrationAction::FixedWorkspaceInheritance { path: cargo_path });
        }
...
"#,
    ...
    fn generate_description(&self, crate_path: &Path) -> Result<String> {
        ...
        let description = match crate_name.as_str() {
            name if name.contains("core") => format!(
                "Core {} functionality for Longhorn Game Engine",
                name.replace("engine-", "")
                    .replace("-core", "")
                    .replace('-', " ")
            ),
            name if name.contains("impl") => format!(
                "{} implementation for Longhorn Game Engine",
                name.replace("engine-", "")
                    .replace("-impl", "")
                    .replace('-', " ")
            ),
            name if name.contains("import") => format!(
                "{} functionality for Longhorn Game Engine",
                name.replace("engine-", "").replace('-', " ")
            ),
            name if name.contains("bridge") => format!(
                "{} for Longhorn Game Engine",
                name.replace("engine-", "").replace('-', " ")
            ),
            _ => format!(
                "{} for Longhorn Game Engine",
    ...
    fn convert_to_workspace_inheritance(&self, cargo_path: &Path) -> Result<()> {
        ...
        for line in &mut lines {
            if line.trim() == "[package]" {
                ...
                {
                    *line = "version.workspace = true".to_string();
                } else if line.starts_with("edition")
                    && line.contains('=')
                    && !line.contains("workspace")
                {
                    *line = "edition.workspace = true".to_string();
                } else if line.starts_with("license")
                    && line.contains('=')
                    && !line.contains("workspace")
                {
                    *line = "license.workspace = true".to_string();
                }
            }
        }
...
pub enum MigrationAction {
    ...
    FixedWorkspaceInheritance {
        path: std::path::PathBuf,
    },
    WouldFixWorkspaceInheritance {
        path: std::path::PathBuf,
    },
    Skipped {
        reason: String,
...
mod tests {
    use super::*;
    use crate::{CrateStructure, StructureValidator};
...
description = "Test crate"
        ...
        assert!(migration_report
            .actions
            .iter()
            .any(|a| matches!(a, MigrationAction::WouldCreate { .. })));
    }
...
license.workspace = true
        ...
        // Fix with migrator
        let migrator = StructureMigrator::new(CrateStructure::default(), false);
        let _migration_report = migrator.fix_crate(temp.path(), &report).unwrap();
