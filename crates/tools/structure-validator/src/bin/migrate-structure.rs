...
use structure_validator::{CrateStructure, StructureValidator, migration::StructureMigrator};
...
fn main() -> Result<()> {
    ...
    println!("Mode: {}", if dry_run { "DRY RUN" } else { "APPLY CHANGES" });
    println!();
    ...
    println!("Found {} crates with issues", 
        validation_report.reports.iter().filter(|r| !r.is_valid()).count());
    println!();
