...
use engine_components_3d::{Camera, Mesh, MeshType, ProjectionType, Transform};
use engine_ecs_core::ecs_v2::{Entity, World};
use engine_renderer_core::{CameraProvider, Renderable, RenderableQuery, TransformProvider};
...
impl CameraProvider for EcsCameraProvider {
    ...
    fn projection_matrix(&self) -> Mat4 {
        ...
        match self.camera.projection_type {
            ProjectionType::Perspective => Mat4::perspective_rh(
                fov_radians,
                aspect,
...
impl EcsRenderBridge {
        ...
        R: FnMut(&EcsRenderableQuery, Option<&dyn CameraProvider>),
    {
        ...
        renderer(
            &renderables,
            camera.as_ref().map(|c| c as &dyn CameraProvider),
        );
    }
}
...
mod tests {
    ...
    fn create_test_world() -> World {
        ...
        world
            .add_component(entity1, Transform::new().with_position(1.0, 0.0, 0.0))
            .unwrap();
        world
            .add_component(
                entity1,
        ...
        world
            .add_component(camera_entity, Transform::new().with_position(0.0, 0.0, 5.0))
            .unwrap();
        world
            .add_component(camera_entity, Camera::perspective(60.0, 0.1, 1000.0))
            .unwrap();
    ...
    fn test_ecs_renderable_query_from_world() {
        ...
        let with_meshes: Vec<_> = renderables
            .iter()
    ...
    fn test_world_with_transforms_only() {
        ...
        for i in 0..3 {
            let entity = world.spawn();
            world
                .add_component(entity, Transform::new().with_position(i as f32, 0.0, 0.0))
                .unwrap();
        }
